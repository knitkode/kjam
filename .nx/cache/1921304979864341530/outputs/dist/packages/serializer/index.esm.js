...
function extractMeta(filepath, i18n) {
    ...
    const dir = normalisePathname(pathParts.slice(0, -1).join("/").replace(/\./g, ""));
...
class Serializer {
    ...
    async run() {
        ...
        this.translations = await getTranslations(join(this.root, this.pathTranslations), this.i18n, this.routes);
    ...
    async getRouting(markdownFiles) {
        ...
        for (let i = 0; i < markdownFiles.length; i++) {
            ...
            const matter = extractMatter(join(this.root, filepath));
    ...
    getSlugsForPath(path) {
        ...
        for (let i = 0; i < this.i18n.locales.length; i++) {
            ...
            const pageEntry = join(this.root, 
            // PAGES:
