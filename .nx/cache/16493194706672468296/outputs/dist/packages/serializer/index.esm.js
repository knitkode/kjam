import { existsSync, readdirSync, readFile, emptyDirSync, mkdirSync, writeFileSync, readFileSync } from 'fs-extra';
import { read } from 'gray-matter';
import { load, JSON_SCHEMA } from 'js-yaml';
import { normalisePathname, ApiGithub, isTestEnv } from '@kjam/core';
...
function extractMeta(filepath, i18n) {
    ...
    var dir = normalisePathname(pathParts.slice(0, -1).join("/").replace(/\./g, ""));
...
function extractRoute(meta, matter) {
    ...
    // normalize the slug
    var slug = normalisePathname(slugFromMatter || slugFromDir || "");
    // special homepage case
    slug = slug === "home" ? "" : slug;
    var templateSlug = normalisePathname("".concat(parentDirs, "/").concat(slug));
 ...
 */ function isCollectionPath(fullpath) {
    ...
    var children = readdirSync(fullpath, {
        withFileTypes: true
    });
 ...
 */ function getTranslatedLink(raw, entry, api, urls) {
    var _urls_id;
    var id = "";
    var _parseUrl = parseUrl(raw), path = _parseUrl.path, relative = _parseUrl.relative, file = _parseUrl.file, query = _parseUrl.query;
    if (!relative) {
        return raw;
    }
    var startsWithDot = path[0] === ".";
    if (startsWithDot) {
        var relativePath = path.replace(/\/index\..+/, "");
        ...
        return _ts_generator$1(this, function(_state) {
            switch(_state.label){
                ...
                case 1:
                    ...
                    locale = locales[i];
                    target = join(folderPath, "".concat(locale, ".yml"));
                    data = void 0;
                    _state.label = 2;
                case 2:
                    _state.trys.push([
...
var Serializer = /*#__PURE__*/ function() {
   ...
   */ _define_property(this, "mdPaths", void 0);
        ...
        this.api = new ApiGithub(_object_spread({
            username: username,
            repo: repo,
            branch: branch
                    ...
                    return _ts_generator(this, function(_state) {
                        switch(_state.label){
                            ...
                            case 2:
                                ...
                                return [
                                    4,
                                    getTranslations(join(_this.root, _this.pathTranslations), _this.i18n, _this.routes)
                                ];
                            case 3:
   ...
   */ function getPaths(dir) {
                ...
                return _async_to_generator(function() {
                    ...
                    return _ts_generator(this, function(_state) {
                        switch(_state.label){
                            case 0:
                                crawler = new fdir().withFullPaths().crawl(dir || _this.root);
                                return [
                                    4,
                                    crawler.withPromise()
                                ];
                            ...
                            case 1:
                                ...
                                return [
                                    ...
                                    paths.filter(filterMarkdownFiles).map(function(filepath) {
                                        return relative(_this.root, filepath);
   ...
   */ function ensureMetaFolder() {
                var target = join(this.root, ".kjam");
                emptyDirSync(target);
                mkdirSync(target, {
                    recursive: true
                });
        ...
        },
        {
            ...
            value: function writeFile() {
                ...
                var target = join(this.root, ".kjam", "".concat(filepath, ".json"));
                var dir = dirname(target);
                var content = JSON.stringify(data, null, isTestEnv() ? 2 : 0);
                if (!existsSync(dir)) {
                    mkdirSync(dir, {
                        recursive: true
                    });
        ...
        },
        {
            ...
            value: function getI18n() {
                var target = join(this.root, this.pathI18n);
                if (existsSync(target)) {
                    var content = readFileSync(target, "utf-8");
                    var i18n = load(content);
                    return i18n;
                }
                return {
                    locales: [
   ...
   */ function getRouting(markdownFiles) {
                        ...
                        switch(_state.label){
                            case 0:
                                ...
                                for(i = 0; i < markdownFiles.length; i++){
                                    ...
                                    matter = extractMatter(join(_this.root, filepath));
                                    // ability to filter out contents with conventional frontmatter flags
                                    if (matter.data.draft) {
                                        continue;
                                    }
                                    isCollection = isCollectionPath(join(_this.root, id));
        ...
        {
            key: "getSlugsForPath",
            value: /** @private */ function getSlugsForPath(path) {
                var pathSlugs = {};
                for(var i = 0; i < this.i18n.locales.length; i++){
                    ...
                    var pageEntry = join(this.root, // PAGES:
                    ...
                    if (existsSync(pageEntry)) {
                        ...
                        var collectionEntry = join(this.root, path, filename);
                    ...
                    // PAGES:
                    slug = slug.startsWith("pages/") ? slug.replace("pages/", "") : slug, pathSlugs[locale] = "/" + normalisePathname(slug);
                }
                return pathSlugs;
            }
        },
